...
class MyDateTime:
    ...
    @staticmethod
    def parse_y_m_d(day: str, hour_offset: int = 0) -> datetime:
        return datetime.strptime(day, "%Y-%m-%d") \
            .replace(tzinfo=timezone(timedelta(hours=hour_offset)))
...
class DateRange:
    ...
    @classmethod
    def from_str(cls, start: str, end: str, step_size: StepSize = StepSize.MONTH):
        return cls(MyDateTime.parse_y_m_d(start), MyDateTime.parse_y_m_d(end), step_size)
...
class DynamoDBClient:
    ...
    def __init__(self, client=None):
        self._table_name = os.environ.get("INV_VAL_CACHE_TABLE_NAME")
        self.client = (
            boto3.client("dynamodb", region_name="us-west-2") if not client else client
        )

    def set_table_name(self, table_name: str) -> "DynamoDBClient":
    ...
    def _to_update_partiql(self, pk_field_name: str, item: dict):
        table_operation = f'UPDATE "{self._table_name}"'
    ...
    def put_item(self, item: dict):
        self.client.put_item(
            TableName=self._table_name, Item=DynamoDBClient._to_attribute_val(item)
        )

    def batch_write_item(self, items: List[dict]) -> list:

        to_put_req_item = lambda x: {
            "PutRequest": {"Item": DynamoDBClient._to_attribute_val(x)}
        }
        batch_transformer = lambda batch: {
            self._table_name: [to_put_req_item(x) for x in batch]
        }
    ...
    def batch_execute_statement(
        self, pk_field_name: str, items: List[Tuple[DBOperation, dict]]
    ):
        ...
        def batch_transformer(batch: List[Tuple[DynamoDBClient.DBOperation, dict]]):
            ...
            return [
                (
                    to_statement(
                        *self._to_update_partiql(pk_field_name, item)
                    )
...
class InventoryValueCache:
    ...
    def write_db(self):
        try:
            self._dynamodb_client.put_item(self._to_write_db_input())
        except Exception as e:
            logging.exception(
    ...
    @staticmethod
    def batch_write_db(
        client: DynamoDBClient, caches: List["InventoryValueCache"]
    ):
        if len(caches) == 0:
            return
        if len(caches) == 1:
        ...
        unprocessed_list = client.batch_write_item(items)
        if unprocessed_list:
            logging.warning(
...
class CacheProvider:
    ...
    def _renew(self, expired_date: datetime):
        ...
        InventoryValueCache.batch_write_db(
            self._factory.dynamodb_client, expired_caches
        )
    ...
    def _get_expired_date(self) -> Optional[datetime]:
        ...
        try:
            earliest_expired_date = MyDateTime.parse_y_m_d(resp[earliest_exp], TZ_UTC_HOUR_OFFSET)
        except ValueError:
            logging.exception(
...
async def lifespan(app: FastAPI):
    ...
    app.state.factory = Factory(
        app.state.graphql_client, app.state.dynamodb_client, app.state.inventory
    )
    yield

app = FastAPI(lifespan=lifespan)

@app.get("/inventory/value/{item_id}")
async def get_inventory_values(item_id: str, startInclusive: str, endExclusive: str, stepSize: StepSize):
    try:
        date_range = DateRange.from_str(startInclusive, endExclusive, stepSize)
        requested_start_date, _ = date_range.get_bounds()
        cache_provider = CacheProvider(requested_start_date, app.state.factory)

        item = app.state.inventory[item_id]
        ...
        msg = f"Error getting inventory values for item {item_id}"
        logging.exception(msg, exc_info=True)
        raise HTTPException(status_code=500, detail=msg)

@app.post("/async/inventory/value", status_code=202)
async def create_monthly_inventory_val_report():
    try:
        end = MyDateTime.curr_month_start_in_tz(TZ_UTC_HOUR_OFFSET)
        start = end - relativedelta(months=1)
        date_range = DateRange(start, end, StepSize.MONTH)

        logging.info(f"Starting async inventory value report for {date_range}")

        cache_provider = CacheProvider(start, app.state.factory)
        prev_cache = cache_provider.get_previous_cache()
        inventory_calculator = InventoryValueCalculator(
            date_range, app.state.factory, prev_cache
        ).calculate()
        new_caches = inventory_calculator.values()
        InventoryValueCache.batch_write_db(
            app.state.dynamodb_client, new_caches
        )
        logging.info(f"Completed async inventory value report for {date_range}")

        return {"message": "Report completed"}
    except HTTPException:
        raise
    except Exception as e:
        msg = "Failed to start async inventory value report"
        logging.exception(msg, exc_info=True)
        raise HTTPException(status_code=500, detail=msg)
